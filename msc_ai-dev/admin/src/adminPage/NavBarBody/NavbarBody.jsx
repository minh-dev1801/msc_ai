...
import { Button, Form, Input, List, Row, Col, Card, Select, message, Layout, Divider, Radio, Pagination } from "antd";
...
const NavbarBody = () => {
  ...
  const fetchVendorStatistics = async(year = null, top = null) =>{
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/ai/vendor-statistics",{
        params,
        headers: { Authorization: `Bearer ${keycloak.token}` },
      });
  ...
  const handleGenerateChart = async () => {
    ...
    const isVendorStatsPrompt = customPrompt.toLowerCase().includes("thống kê") && 
          ...
          >
            ...
            <div style={{ marginBottom: 16, color: '#666', fontSize: '14px' }}>
              ...
              {data.length > pageSize && (
                <> • Hiển thị: <strong>{getCurrentPageData().length}</strong> items</>
              )}
            </div>
                ...
                <List.Item>
                  ...
                  {showVendorStats ? (
                    <Row style={{ width: '100%' }}>
                      ...
                      <Col span={6}><strong>{item.value} lần</strong></Col>
                      <Col span={6}><strong>{item.percentage}%</strong></Col>
                    </Row>
